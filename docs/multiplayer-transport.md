...
| **Pusher Channels** | as above | per delivery | **none** — Cache Channels remember only the last event, for up to 30 minutes, in memory; the vendor's own advice is that durable state belongs in your database | 200K messages/day, 100 connections | equivalent to Ably, tighter free tier, and no replay at all |
| **PubNub** | as above | **per monthly active user**, not per message | paid, and `signal` messages are never stored | 200 MAU | **it does not use WebSockets** — delivery is HTTP long-polling with a 280-second hold and a 310s client read timeout, which is architecturally closer to this relay than to a socket; the MAU unit fits a chat product better than a two-player game |
...
| **Liveblocks** | a *connectable* (Third Party) listing rather than a native one — env vars are injected, billing is not | metered collaboration minutes, not messages | **24 hours** on Free, 7 days on Pro | 3,000 collaboration minutes, 500 rooms, 10 connections per room | the best SvelteKit-native presence story and 10 connections covers players plus spectators, but no unified billing and the event log is far too short to be the game's history |
