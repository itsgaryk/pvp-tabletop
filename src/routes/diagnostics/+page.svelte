...
<svelte:head>
   <title>Diagnostics - PvP Tabletop</title>
</svelte:head>

<div class="p-4 max-w-[1100px] mx-auto flex flex-col gap-4">
   <header class="flex items-center gap-3 flex-wrap">
      <h1 class="text-xl font-bold">Diagnostics</h1>
      <span class="text-sm text-[var(--text-color-two)]">this browser, live</span>
      <span class="flex-1"></span>
      <button class="px-2 py-1 text-sm rounded-md bg-[var(--bg-color-two)]" on:click={() => checkHealth(true)}>
         Probe store
      </button>
      <button class="px-2 py-1 text-sm rounded-md bg-[var(--bg-color-two)]" on:click={copyReport}>
         {copied ? 'Copied' : 'Copy report'}
      </button>
      <a class="px-2 py-1 text-sm rounded-md primary" href="/">Back to the board</a>
   </header>

   <p class="text-xs text-[var(--text-color-two)]">
      Snapshot taken every {REFRESH_MS} ms. "Probe store" makes the relay write and expire a key
      (one command) - use it when something is wrong, not repeatedly. The relay's own view of a
      room is a separate question: <code>node tools/room-log.mjs &lt;ROOM&gt;</code>.
   </p>
   ...
   {:else}
      ...
      <section class="card">
         <h2>
            ...
            <button class="px-2 py-0.5 ml-2 text-xs rounded-md bg-[var(--bg-color-two)]" on:click={clearRelayEvents}>clear</button>
         </h2>
